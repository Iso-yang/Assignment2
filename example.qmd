...
Inside the project, I created a file `example.qmd` that can be knitted into a HTML file. ![](images/HTML_file.jpg)
...
git checkout -b testbranch: Create a new branch called testbranch . git add example.qmd git commit -m "Modified qmd in testbranch" git push origin testbranch: Pushed the testbranch branch to the remote repository.

# Step 4: Add data folder and amend commit

Create a folder called data, and add the data from Assignment 1 to that folder. git add . git commit -m"create a folder" git push origin testbranch.
